...
RESULT_FOLDER = os.path.join(SAVE_PASH, "results_cross_correlation")
...
def get_all_models(t_exp, m_exp, theory_files):
    ...
    all_results.sort(key=lambda x: x['cross_correlation'], reverse=False)
    return all_results
...
def plot_all_results(exp_name, all_models, stats):
    ...
    # 绘制最佳模型（红线）
    label_val = f"SMAPE={best['cross_correlation']:.2f}" if 'cross_correlation' in best else f"cross_correlation={best['cross_correlation']:.2e}"
    ...
    ax_main.set_title(f"{exp_name} - cross_correlation Сравнение моделей")
    ...
    all_errors = [m['cross_correlation'] for m in all_models] if 'cross_correlation' in best else [m['cross_correlation'] for m in all_models]
    ax_hist.hist(all_errors, bins=20, color='skyblue', edgecolor='white')
    ax_hist.axvline(all_errors[0], color='red', linestyle='--')
    
    # 绿线标注前 10% 阈值
    if 'cross_correlation' in best:
