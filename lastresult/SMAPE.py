...
RESULT_FOLDER = os.path.join(SAVE_PASH, "results_smape")
...
#SMAPE
def smape(obs, pred, fraction=0.75, eps=1e-8):
    obs = np.array(obs)
    pred = np.array(pred)
    ...
    return 200 * np.mean(np.abs(obs - pred) / denominator)
...
def get_all_models(t_exp, m_exp, theory_files):
    ...
    for file in theory_files:
        ...
        # 【修改点2】SMAPE 计算
        error = smape(y_exp_win, y_th_win)
    ...
    # 在 get_all_models 函数循环内
        error = smape(y_exp_win, y_th_win)
        
        if error < 1e-10: # 接近于 0
            print(f"--- DETECTED ZERO ERROR ---")
            print(f"Exp Window Sum: {np.sum(y_exp_win)}")
            print(f"Theory Window Sum: {np.sum(y_th_win)}")
            print(f"File: {file}")
    all_results.sort(key=lambda x: x['smape'], reverse=False)
    return all_results
...
def plot_all_results(exp_name, all_models, stats):
    ...
    # 绘制最佳模型（红线）
    label_val = f"SMAPE={best['smape']:.2f}" if 'smape' in best else f"smape={best['smape']:.2e}"
    ...
    ax_main.set_title(f"{exp_name} - smape Сравнение моделей")
    ...
    all_errors = [m['smape'] for m in all_models] if 'smape' in best else [m['smape'] for m in all_models]
