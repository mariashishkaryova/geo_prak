...
RESULT_FOLDER = os.path.join(SAVE_PASH, "results_r_squared")
...
# R^2
def r_squared(obs, pred, fraction=0.75):
    obs = np.array(obs)
    pred = np.array(pred)
...
def get_all_models(t_exp, m_exp, theory_files):
    ...
    for file in theory_files:
        ...
        # 【修改点2】SMAPE 计算
        error = r_squared(y_exp_win, y_th_win)
...
def plot_all_results(exp_name, all_models, stats):
    ...
    # 绘制最佳模型（红线）
    label_val = f"R²={best['r_squared']:.2f}" if 'r_squared' in best else f"r_squared={best['r_squared']:.2e}"
    ...
    ax_main.set_title(f"{exp_name} - r_squared Сравнение моделей")
    ...
    all_errors = [m['r_squared'] for m in all_models] if 'r_squared' in best else [m['r_squared'] for m in all_models]
    ax_hist.hist(all_errors, bins=20, color='skyblue', edgecolor='white')
    ax_hist.axvline(all_errors[0], color='red', linestyle='--')
    
    # 绿线标注前 10% 阈值
    if 'r_squared' in best:
