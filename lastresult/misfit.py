...
RESULT_FOLDER = os.path.join(SAVE_PASH, "results_misfit")
...
def get_all_models(t_exp, m_exp, theory_files):
    ...
    for file in theory_files:
        ...
        # 【修改点2】SMAPE 计算
        error = misfit(y_exp_win, y_th_win)
    ...
    all_results.sort(key=lambda x: x['misfit'], reverse=False)
    return all_results
...
def plot_all_results(exp_name, all_models, stats):
    ...
    # 绘制最佳模型（红线）
    label_val = f"R²={best['misfit']:.2f}" if 'misfit' in best else f"misfit={best['misfit']:.2e}"
    ...
    ax_main.set_title(f"{exp_name} - misfit Сравнение моделей")
    ...
    all_errors = [m['misfit'] for m in all_models] if 'misfit' in best else [m['misfit'] for m in all_models]
