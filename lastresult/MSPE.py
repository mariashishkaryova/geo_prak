...
RESULT_FOLDER = os.path.join(SAVE_PASH, "results_mspe")
...
# MSPE
def mspe(obs, pred, eps=1e-8):
    obs = np.array(obs)
    pred = np.array(pred)
    
    # 这里的关键是除以 obs 的平方，体现“Percentage”
    # 使用 eps 防止分母为 0
    denominator = np.square(obs)
    denominator = np.where(denominator < eps, eps, denominator)
    
    return np.mean(np.square(pred - obs) / denominator)
...
def get_all_models(t_exp, m_exp, theory_files):
    ...
    all_results.sort(key=lambda x: x['mspe'], reverse=False)
    return all_results
...
def plot_all_results(exp_name, all_models, stats):
    ...
    # 绘制最佳模型（红线）
    label_val = f"MSPE={best['mspe']:.2f}" if 'mspe' in best else f"mspe={best['mspe']:.2e}"
    ...
    ax_main.set_title(f"{exp_name} - mspe Сравнение моделей")
    ...
    all_errors = [m['mspe'] for m in all_models] if 'mspe' in best else [m['mspe'] for m in all_models]
