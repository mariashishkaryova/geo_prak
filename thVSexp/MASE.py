...
RESULT_FOLDER = os.path.join(SAVE_PASH, "results of MASE")
...
# MASE
def mase(obs, pred, fraction=0.75, eps=1e-8):
    obs = np.array(obs)
    pred = np.array(pred)
    ...
    # MAE
    mae_model = np.mean(np.abs(obs - pred))

    naive = np.abs(obs[1:] - obs[:-1])
...
def main():
    ...
    for exp in experiment_dirs:
        ...
        for f,_,_,e,_,_ in best_models:
            ...
            print(os.path.basename(f),"mase =",e)
