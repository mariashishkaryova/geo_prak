...
RESULT_FOLDER = os.path.join(SAVE_PASH, "results of MAE")
...
def mae_criterion(obs, pred):
    # obs 和 pred 已经是 prepare_comparison_data 处理好的等长数组
    if len(obs) == 0: return float('inf')
...
def find_best_models(t_exp, m_exp, theory_files):
    ...
    for file in theory_files:
        ...
        error = mae_criterion(y_exp_win, y_th_win) 
...
def main():
    ...
    for exp in experiment_dirs:
        ...
        for f,_,_,e,_,_ in best_models:
            ...
            print(os.path.basename(f),"mae_criterion =",e)
