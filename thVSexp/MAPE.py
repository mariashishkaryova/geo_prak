...
RESULT_FOLDER = os.path.join(SAVE_PASH, "results of MAPE")
...
#MAPE
def mape(obs, pred, fraction=0.75, eps=1e-8):
    obs = np.array(obs)
    pred = np.array(pred)
    ...
    return 100 * np.mean(np.abs((pred - obs) / obs_safe))
...
def main():
    ...
    for exp in experiment_dirs:
        ...
        for f,_,_,e,_,_ in best_models:
            ...
            print(os.path.basename(f),"mape =",e)
